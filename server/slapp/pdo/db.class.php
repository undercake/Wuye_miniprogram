...
class DB {
	...
	public function connect($name = 'master') {
		...
		if(empty($cfg)) {
			exit("The master database is not found, Please checking");
		}
	...
	public function tablename($table) {
		return (((!empty($this->tablepre)) && strpos($table, $this->tablepre) === 0) || strpos($table, 'ims_') === 0) ? $table : "`{$this->tablepre}{$table}`";
	}

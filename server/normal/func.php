...
function make_str( $length = 8, $type = "lun", $ext = array()){
	...
	if (strpos($type,'u') <= -1 && strpos($type,'l') <= -1 && strpos($type,'n') <= -1 && strpos($type,'s') <= -1 && empty($ext)) {$type = 'ln';}
	...
	for($i = 0; $i < $length; $i++) {
		...
		$str = $i == 0 ? $chars[mt_rand(0, ($max > 9 ? 9 : $max))] : $str;
	}
	return $str;
}
...
function dsy_session(){
	session_destroy();
	header('Set-Cookie: '.make_str(4).'=destory');
}
